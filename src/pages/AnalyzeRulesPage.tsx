        ...
        if (fileError) {
            ...
            try {
                await dbService.addAuditLog({
                  ...
                  details: {
                    page: 'AnalyzeRulesPage',
                    fileName: file.name,
                    extractedTextLength: text.length,
                  }
                });
              } catch (logError) {
                console.warn("Failed to write audit log for file extraction:", logError);
              }
        }
      } catch (e: any) {
  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      try {
        await dbService.addAuditLog({
          timestamp: new Date(),
          actionType: 'REGULATION_SUMMARY_GENERATED',
          details: {
            page: 'AnalyzeRulesPage',
            documentName: currentFileName,
            documentLength: regulationDocumentText.length,
            summaryLength: summary.length,
          }
        });
      } catch (logError) {
        console.warn("Failed to write audit log for summary generation:", logError);
      }
    } catch (e: any) {
  ...
  const handleAskQuestion = async () => {
    ...
    try {
      ...
      try {
        await dbService.addAuditLog({
          timestamp: new Date(),
          actionType: 'REGULATION_QA_ANSWERED',
          details: {
            page: 'AnalyzeRulesPage',
            documentName: currentFileName,
        ...
        console.warn("Failed to write audit log for Q&A generation:", logError);
      }
      setCurrentQuestion('');
    } catch (e: any) {
        ...
        <TextArea
          ...
          onChange={(e) => {
            ...
            if(e.target.value.trim()) {
                setError(null); 
                setFileGuidance(null); 
                setCurrentFileName(null); // Pasted content, no file name
            }
